...
import { Search, Menu, X, Plus, User, Book, Calendar, FileText, Home, Users } from 'lucide-react';
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-secondary text-white transition-all duration-300 flex flex-col`}>
        <div className="p-4 flex items-center justify-between">
          <h1 className={`font-bold text-xl ${sidebarOpen ? 'block' : 'hidden'}`}>CM</h1>
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          {filteredCourses.map((c, i) => (
            <button key={i} className="w-full text-left py-2 px-3 hover:bg-primary rounded transition flex items-center space-x-2">
              <Book size={16} />
              <span className={`${sidebarOpen ? 'block' : 'hidden'}`}>{c.name}</span>
            </button>
          ))}
          <button className="w-full text-left py-2 px-3 hover:bg-primary rounded transition flex items-center space-x-2">
            <Plus size={16} />
            <span className={`${sidebarOpen ? 'block' : 'hidden'}`}>New Course</span>
          </button>
        </nav>
      </aside>
        ...
        <header className="bg-secondary text-white p-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              <Search className="absolute left-3 top-3 text-white opacity-50" size={18} />
              <input 
                type="text" 
            ...
            <button className="bg-accent text-white px-2 py-1 rounded">+</button>
        ...
        <main className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="flex justify-center space-x-4 text-sm">
              <div>Completed: 150</div>
              <div>In Progress: 280</div>
              <div>Not Started: 110</div>
            </div>
          </div>

          {/* Faculty Workload Bars */}
          <div className="bg-white rounded-lg shadow p-4">
                  ...
                  <tr key={i}>
                    ...
                    <td>
                      <div className="bg-gray-200 h-2 rounded-full">
                        <div className="bg-primary h-2 rounded-full" style={{ width: `${p.percent}%` }}></div>
                      </div>
                    </td>
                  </tr>
                ))}
